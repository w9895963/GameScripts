...
public class I_Placing : IC_Base {
    ...
    void Update () {
       Global.Function.CallFn(this).DrawLine (FindObjectOfType<CM_MainCharacter> ().transform.position, setting.targetPosition, 0.01f);
    }
    void OnEnable () {


        data.tempInstance.AddIfEmpty (0,
            () =>Global.Function.CallFn(this).AddGlobalPointerEvent (PointerEventType.onMoveNotDrag, (d) => {
        ...
        if (setting.endCondition.click) {
            data.tempInstance.AddIfEmpty (1,
                () =>Global.Function.CallFn(this).AddGlobalPointerEvent (PointerEventType.onClick, (d) => {
                    enabled = false;
                })
            );
        }
